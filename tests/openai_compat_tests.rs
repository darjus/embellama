...
async fn test_openai_response_structure() {
    ...
    assert_eq!(first_item["object"], "embedding", "Data item object must be 'embedding'");
    assert_eq!(first_item["index"], 0, "Index must be 0 for first item");
    assert!(first_item["embedding"].is_array(), "Embedding must be an array");
    
    // Validate usage structure
    let usage = &body["usage"];
    assert!(usage["prompt_tokens"].is_number(), "prompt_tokens must be a number");
    assert!(usage["total_tokens"].is_number(), "total_tokens must be a number");
...
async fn test_openai_base64_encoding_format() {
    ...
    assert_eq!(decoded_bytes.len() % 4, 0, "Decoded bytes should be multiple of 4 (f32 size)");
}
...
async fn test_openai_error_response_format() {
    ...
    assert!(body["error"].is_object(), "Error response must contain 'error' object");
    
    let error = &body["error"];
    assert!(error["message"].is_string(), "Error must have 'message' field");
    ...
    if error.get("code").is_some() {
        assert!(error["code"].is_string(), "Error 'code' must be string if present");
    }
}
...
async fn test_openai_models_list_format() {
    ...
    assert_eq!(body["object"], "list", "Models response object must be 'list'");
    ...
    for model in models {
        assert_eq!(model["object"], "model", "Each item must have object='model'");
        assert!(model["id"].is_string(), "Model must have 'id' field");
        assert!(model["created"].is_number(), "Model must have 'created' timestamp");
        assert!(model["owned_by"].is_string(), "Model must have 'owned_by' field");
    }
}
...
async fn test_openai_embedding_dimensions() {
    ...
    assert!(embedding.len() >= 64, "Embedding should have reasonable dimensions");
